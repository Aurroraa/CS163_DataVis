...
void AVLState::DrawDeleteMenu(float x, float y) {
    ...
    if (UIHelper::DrawModernBtn({x + 10, y + 70, 150, 30}, "Delete Value", true, config) || IsKeyPressed(KEY_ENTER)) {
        if (inputBuffer[0] != '\0') {
            Visualizer::Instance().ClearHistory();
            Visualizer::Instance().RecordState("Initial State", 0, avl.captureState(), {});
...
void AVLState::DrawSearchMenu(float x, float y) {
    ...
    if (UIHelper::DrawModernBtn({x + 10, y + 70, 150, 30}, "Search Value", true, config) || IsKeyPressed(KEY_ENTER)) {
        if (inputBuffer[0] != '\0') {
            Visualizer::Instance().ClearHistory();
            Visualizer::Instance().RecordState("Initial State", 0, avl.captureState(), {});
